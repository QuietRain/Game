class DisplayFacory {
	...
	createPropertyButton(PropertyButton: egret.Bitmap, panel: egret.Bitmap, stage: egret.DisplayObjectContainer, hero: Hero, x: number, y: number): egret.DisplayObjectContainer {
		var ButtonContainer = new egret.DisplayObjectContainer;
		PropertyButton.touchEnabled = true;

        PropertyButton.addEventListener(egret.TouchEvent.TOUCH_TAP, () => {
			...
			if (locked == 0) {
				...
				closefield.addEventListener(egret.TouchEvent.TOUCH_TAP, () => {
	...
	createHero(hero: Hero, x, y): egret.DisplayObjectContainer {
		var _y = 0;
		var container = new egret.DisplayObjectContainer();
	...
	createEquitment(equipments: Equipment[], x, y): egret.DisplayObjectContainer {
		var _y = 0;
		var container = new egret.DisplayObjectContainer();
	...
	createPropertyPanel(properties: Properties, stage: egret.DisplayObjectContainer, x, y) {
		var locked = User.user.statemachine.locked;
		locked++;
	...
	createMonsterModel(model: egret.DisplayObjectContainer, scale: number, stage: egret.DisplayObjectContainer, x: number, y: number) {
